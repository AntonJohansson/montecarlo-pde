...
inline void random_default_seed_xors(struct random_series_xors *series) {
    series-> state = (u32_4x) {78953890, 235498, 893456, 93453080};
}

inline u32_4x random_next_u32_4x_xors(struct random_series_xors *series) {
...
inline u32 random_next_u32_xors(struct random_series_xors *series) {
    return random_next_u32_4x_xors(series)[0];
}

inline f32 random_unilateral_xors(struct random_series_xors *series) {
    f32 divisor = 1.0f / (f32) UINT32_MAX;
    f32 result = divisor * (f32) random_next_u32_xors(series);
    return result;
}

inline f32 random_bilateral_xors(struct random_series_xors *series) {
    return 2.0f*random_unilateral_xors(series) - 1.0f;
}

inline f32 random_f32_xors(struct random_series_xors *series, f32 min, f32 max) {
    f32 divisor = 1.0f / (f32) UINT32_MAX;
...
inline void random_seed_pcg(struct random_series_pcg *series, u64 state, u64 selector) {
    series->state = state;
    series->selector = (selector << 1) | 1;
}

inline void random_default_seed_pcg(struct random_series_pcg *series) {
    random_seed_pcg(series, 7123915, 912305);
}

inline u32 rotate_right(u32 value, i32 amount) {
    amount &= 31;
    return (value >> amount) | (value << (32 - amount));
}

inline u32 random_next_u32_pcg(struct random_series_pcg *series) {
...
inline f32 random_unilateral_pcg(struct random_series_pcg *series) {
    f32 divisor = 1.0f / (f32) UINT32_MAX;
    return divisor * (f32) random_next_u32_pcg(series);
}

inline f32 random_bilateral_pcg(struct random_series_pcg *series) {
    return 2.0f*random_unilateral_pcg(series) - 1.0f;
}

inline f32 random_f32_pcg(struct random_series_pcg *series, f32 min, f32 max) {
    f32 divisor = 1.0f / (f32) UINT32_MAX;
