...
struct gk_data {
    real kronod_nodes[MAX_GAUSS_POINTS+1];
    real kronod_weights[MAX_GAUSS_POINTS+1];
    real gauss_weights[(MAX_GAUSS_POINTS+1)/2];
    u32 kronod_size;
    u32 gauss_size;
...
typedef void integrand_func(real * restrict, const real * restrict, const u32, void *);

typedef real coord_transform_input_func(real x, real a, real b);
typedef real coord_transform_output_func(real x);
...
static inline real norm(real r) {
    return r*r;
}
...
static void evaluate_rule(integrand_func *f, real start, real end, const struct quadgk_settings *settings, real * restrict sample_points, real * restrict transformed_sample_points, real * restrict sample_output, struct eval_result *res) {
    const struct gk_data *gk = &settings->gk;
    const real half_length = 0.5 * (end - start);

    //
    // 	0 -- if even size
    //	1 -- if odd size
    //
    const u32 is_order_odd = 1 - (gk->kronod_size & 1);
    ...
    const u32 size = gk->gauss_size - is_order_odd;

    // Compute all sample points
    for (u32 i = 0; i < size; ++i) {
        real xg = gk->kronod_nodes[2*i+1];
        real xk = gk->kronod_nodes[2*i];

        sample_points[4*i + 0] = start + (1-xg)*half_length;
        sample_points[4*i + 1] = start + (1+xg)*half_length;
        sample_points[4*i + 2] = start + (1-xk)*half_length;
        sample_points[4*i + 3] = start + (1+xk)*half_length;
    }
    sample_points[4*size + 0] = start + half_length;
    sample_points[4*size + 1] = start + (1-gk->kronod_nodes[gk->kronod_size-2])*half_length;
    sample_points[4*size + 2] = start + (1+gk->kronod_nodes[gk->kronod_size-2])*half_length;
    ...
    real Ig = 0, Ik = 0;
    for (u32 i = 0; i < size; ++i) {
        const real fg = sample_output[4*i + 0] + sample_output[4*i + 1];
        const real fk = sample_output[4*i + 2] + sample_output[4*i + 3];
    ...
    const real Ik_s = Ik*half_length;
    const real Ig_s = Ig*half_length;
    const real error = norm(Ik_s - Ig_s);
...
u64 quadgk_required_memory(const struct quadgk_settings *settings) {
    return sizeof(struct segment) * (settings->max_iters+1) + 3*sizeof(real)*settings->gk.sample_size;
}

u32 quadgk_infinite_interval(integrand_func *f, const struct quadgk_settings *settings, void *memory, struct quadgk_result *res) {
    if (!settings) {
        log_error("You need to provide a quadgk_settings struct!");
        return 1;
    }

    if (!res) {
        log_error("You need to provide a quadgk_result struct!");
        return 1;
    }

    if (settings->gk.gauss_size == 0) {
        log_error("You need to specify a gk rule!");
        return 1;
    }
    ...
    real *sample_points = (real *) ((u8 *) memory + sizeof(struct segment) * (settings->max_iters+1));
    real *transformed_sample_points = sample_points + settings->gk.sample_size;
    real *sample_output = transformed_sample_points + settings->gk.sample_size;
    ...
    struct prioqueue pq = {
        .item_size = sizeof(struct segment),
        .max_items = settings->max_iters + 1,
        .memory = memory,
    ...
    struct eval_result left_eval_res  = {0};
    struct eval_result right_eval_res = {0};

    while (!should_exit(settings, res)) {
        prioqueue_pop(&pq, &largest_error_seg);

        const real midpoint = 0.5 * (largest_error_seg.start + largest_error_seg.end);
